...
class Carrito:
    ...
    def aplicar_descuento_condicion(self, porcentaje, minimo):
        """
        Aplica descuento si el total excede al mínimo
        No se aplica nada si el total no excede al mínimo
        """
        total = self.calcular_total()
        if total >= minimo:
            descuento = total * porcentaje / 100
    ...
    def obtener_items_ordenados(self, criterio: str):
        """
        Devuelve la lista de productos en el carrito ordenados segun criterio
        """
        if criterio == "precio":
            ...
            raise ValueError("Criterio no válido. Usar 'precio' o 'nombre'.")
