...
function gestionar_ramas() {
    while true; do
        ...
        echo "f) Renombrar una rama: "  # Creando la nueva opcion
        echo -n "Seleccione una opción: "
        read opcion_rama
        case "$opcion_rama" in
            a|A)
            ...
            e|E)
                ...
                ;;
            f|F)
                echo -n "Ingrese el nombre de la rama a cambiar el nombre (solo ramas locales): "
                read rama  # Nombre de la rama local
                echo -n "Ingrese el nuevo nombre para la rama: "
                read nuevo_nombre  # Nuevo nombre de la rama
                git branch -m "$rama" "$nuevo_nombre"  # Ejecuta 'git branch -m'
...
function gestionar_diff() {
    while true; do
        ...
        echo "d) Volver al menú principal"
        echo "e) Comparar diferencia de un archivo especifico"  # Nueva opcion
        echo -n "Seleccione una opción: "
        read opcion_diff
        case "$opcion_diff" in
            a|A)
            ...
            e|E)
                echo -n "Ingrese el primer identificador (rama o commit): "
                read identificador_1  # Primer commit o rama
                echo -n "Ingrese el segundo identificador (rama o commit): "
                read identificador_2  # Segundo commit o rama
                echo -n "Ingrese la ruta del archivo: "
                read ruta  # Ruta del archivo
                git diff "$identificador_1" "$identificador_2" -- "$ruta"  # Ejecucion del comando
                ;;
            *)
                echo "Opción no válida, intente de nuevo."
                ;;
