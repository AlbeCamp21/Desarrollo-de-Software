class UserManager:
    ...
    def add_user(self, username, password):
        hashed = self.hash_service.hash(password)  # se genera el hash
        self.repo.save_user(username, hashed)  # se guarda el usuario
        if self.email_service:
            self.email_service.send_welcome_email(username)

    def user_exists(self, username):
        return self.repo.exists(username)  # busca si el usuario existe
